...
class UniBoardSSWide(FirmwareBlock):
    ...
    def __init__(self, board, **kwargs):
        ...
        self._nof_select_per_chnl = self._nof_select
        self._adrs_single_chnl    = 2**ceil_log2(self._nof_select)
    ...
    def read_selects(self):
        ...
        for k in range(self._wb_factor):
            addr_offset = self._instance_number * self._adrs_single_chnl * self._wb_factor + k * self._adrs_single_chnl
    ...
    def write_selects(self, data):
        ...
        instance_offset = self._instance_number * self._adrs_single_chnl * self._wb_factor
        for k in range(self._wb_factor):
            addr_offset = instance_offset + k * self._adrs_single_chnl
            self.board.write_register(self._ram_address, data[k * self._nof_select_per_chnl : (k +1) * self._nof_select_per_chnl],
